              ...
              conn.query(
                  ...
                  .then((row) => {
                    ...
                    conn.query(
                            ...
                            [
                              newUsername, newEmail, newForename, newSurname,
                              newBirthday, userId
                            ])
                        .then((result) => {
            ...
            .then((row) => {
              ...
              conn.query(
                  ...
                  .then((row) => {
                    ...
                    conn.query("DELETE FROM User WHERE userId = (?)",
                               [ userId ])
                        .then((result) => {
                          if (result.affectedRows !== 1) {
                            res.status(401).send("something went wrong");
...
function checkUserCredentials(pool, req, res) {
  ...
  pool.getConnection()
      .then((conn) => {
        conn.query("SELECT salt FROM User WHERE username = (?) OR email = (?)",
                   [
                     username,
                     username,
                   ])
            .then((row) => {
              ...
              sessionExpiringDate.setDate(sessionExpiringDate.getDate() +
                                          daysUntilExpiring);
              conn.end();
              conn.query(
...
function requestResetPasswordKey(pool, req, res) {
  ...
  pool.getConnection()
      .then((conn) => {
        conn.query("SELECT salt, userId FROM User WHERE email = (?)", [ email ])
            .then((row) => {
              if (!row[0]) {
                res.status(404).send("user does not exist");
                conn.end();
              ...
              let salt = row[0].salt;
              let userId = row[0].userId;
              let hashedResetPasswordKey =
                  utility.sha512(resetPasswordKey, salt).passwordHash;
            ...
            .then((row) => {
              ...
              conn.query(
                  ...
                  .then((result) => {
                    if (result.affectedRows !== 1) {
                      res.status(401).send(
                          "resetPasswordKey is not valid anymore");
                      conn.end();
                      return;
                    }
                    res.status(202).json(
                        "password has been changed"); // TODO route to login
                    // page
                    conn.end();
                  });
            });
