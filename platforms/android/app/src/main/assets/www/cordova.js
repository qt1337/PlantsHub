...
define.remove = function(id) { delete modules[id]; };
...
define("cordova", function(require, exports, module) {
  ...
  var cordova = {
    ...
    addWindowEventHandler : function(
        event) { return (windowEventHandlers[event] = channel.create(event)); },
    ...
    removeWindowEventHandler : function(
        event) { delete windowEventHandlers[event]; },
    removeDocumentEventHandler : function(
        event) { delete documentEventHandlers[event]; },
    ...
    fireWindowEvent : function(type, data) {
      ...
      if (typeof windowEventHandlers[type] !== 'undefined') {
        setTimeout(function() { windowEventHandlers[type].fire(evt); }, 0);
      } else {
    ...
    // navigating. This way, it's very unlikely that any new callback would get
    // the same callbackId as an old callback.
    ...
    callbackError : function(callbackId, args) {
      ...
      // callbackFromNative. Derive success from status.
    ...
    callbackFromNative : function(callbackId, isSuccess, status, args,
      ...
      try {
        ...
        if (callback) {
          ...
          NO_RESULT which is used to remove a callback from the list without
          calling the callbacks typically keepCallback is false in this case
          */
...
define("cordova/android/nativeapiprovider", function(require, exports, module) {
  ...
  module.exports = {
    ...
    setPreferPrompt : function(value) {
      currentApi =
          value ? require('cordova/android/promptbasednativeapi') : nativeApi;
    },
    // Used only by tests.
...
define(
    "cordova/android/promptbasednativeapi", function(require, exports, module) {
      /**
       * Implements the API of ExposedJsApi.java, but uses prompt() to
       * communicate. This is used pre-JellyBean, where addJavascriptInterface()
       * is disabled.
       */
      ...
      module.exports = {
        exec : function(bridgeSecret, service, action, callbackId, argsJson) {
          return prompt(argsJson, 'gap:' + JSON.stringify([
            bridgeSecret, service, action, callbackId
          ]));
        },
        setNativeToJsBridgeMode : function(
            bridgeSecret,
            value) { prompt(value, 'gap_bridge_mode:' + bridgeSecret); },
...
define("cordova/argscheck", function(require, exports, module) {
   ...
   * `undefined` while uppercase characters allow those values to be passed.
   *
  ...
  function checkArgs(spec, functionName, args, opt_callee) {
    ...
    if (errMsg) {
      ...
      errMsg = 'Wrong type for parameter "' +
               extractParamName(opt_callee || args.callee, i) + '" of ' +
               functionName + ': ' + errMsg;
...
define("cordova/builder", function(require, exports, module) {
      ...
      try {
        ...
        if (clobber) {
          ...
          if (typeof parent[key] === 'undefined') {
            ...
            // Set result to what already exists, so we can build children into
            // it if they exist.
            result = parent[key];
          }
        }
  ...
  exports.buildIntoButDoNotClobber = function(
      objects, target) { include(target, objects, false, false); };
  exports.buildIntoAndClobber = function(
      objects, target) { include(target, objects, true, false); };
  exports.buildIntoAndMerge = function(
      objects, target) { include(target, objects, true, true); };
...
define("cordova/channel", function(require, exports, module) {
   ...
   * page is loaded and parsed. onNativeReady*              Internal event that
   * indicates the Cordova native side is ready. onCordovaReady* Internal event
   * fired when all Cordova JavaScript objects have been created. onDeviceReady*
   * User event fired to indicate that Cordova is ready onResume User event
   * fired to indicate a start/resume lifecycle event onPause User event fired
   * to indicate a pause lifecycle event
   *
   * The events marked with an * are sticky. Once they have fired, they will
   * stay in the fired state. All listeners that subscribe after the event is
   * fired will be executed right away.
   *
   * The only Cordova events that user code should register for are:
   *      deviceready           Cordova native code is initialized and Cordova
   * APIs can be called from JavaScript pause                 App has moved to
   * background resume                App has returned to foreground
   *
  ...
  var channel = {
    ...
    create : function(
        type) { return (channel[type] = new Channel(type, false)); },
    createSticky : function(
        type) { return (channel[type] = new Channel(type, true)); },
  ...
  function checkSubscriptionArgument(argument) {
    if (typeof argument !== 'function' &&
      ...
      throw new Error('Must provide a function or an EventListener object ' +
   ...
   * Returns the guid.
   */
  Channel.prototype.subscribe = function(eventListenerOrFunction,
                                         eventListener) {
    checkSubscriptionArgument(eventListenerOrFunction);
    var handleEvent, guid;

    if (eventListenerOrFunction &&
...
define("cordova/exec", function(require, exports, module) {
  ...
  var channel = require('cordova/channel');
  var jsToNativeModes = {PROMPT : 0, JS_OBJECT : 1};
  var nativeToJsModes = {
  ...
  function androidExec(success, fail, service, action, args) {
    ...
    // By default, we use the failsafe, since addJavascriptInterface breaks too
    // often
    ...
    // bridge mode. This happens in rare circumstances, such as when certain
    // Unicode characters are passed over the bridge on a Galaxy S2.  See
    // CB-2666.
  ...
  function pollOnce(opt_fromOnlineEvent) {
    if (bridgeSecret < 0) {
      // This can happen when the NativeToJsMessageQueue resets the online state
      // on page transitions. We know there's nothing to retrieve, so no need to
      // poll.
      return;
    }
    var msgs = nativeApiProvider.get().retrieveJsMessages(
        bridgeSecret, !!opt_fromOnlineEvent);
    if (msgs) {
      messagesFromNative.push(msgs);
...
define("cordova/init", function(require, exports, module) {
  ...
  if (document.readyState === 'complete' ||
    ...
    document.addEventListener('DOMContentLoaded',
                              function() { channel.onDOMContentLoaded.fire(); },
                              false);
  }
  ...
  // Wrap in a setTimeout to support the use-case of having plugin JS appended
  // to cordova.js. The delay allows the attached modules to be defined before
  // the plugin loader looks for them.
  setTimeout(function() {
    pluginloader.load(function() { channel.onPluginsReady.fire(); });
  }, 0);
...
define("cordova/modulemapper", function(require, exports, module) {
  ...
  exports.runs = function(moduleName) { addEntry('r', moduleName, null); };

  function prepareNamespace(symbolPath, context) {
    if (!symbolPath) {
      return context;
    }
    return symbolPath.split('.').reduce(
        function(cur, part) { return (cur[part] = cur[part] || {}); }, context);
  }
    ...
    for (var i = 0, len = symbolList.length; i < len; i += 3) {
      ...
      var deprecationMsg =
          symbolPath in deprecationMap
              ? 'Access made to deprecated symbol: ' + symbolPath + '. ' +
                    deprecationMsg
              : null;
...
define("cordova/platform", function(require, exports, module) {
  ...
  module.exports = {
    ...
    bootstrap : function() {
      ...
      var APP_PLUGIN_NAME = Number(cordova.platformVersion.split('.')[0]) >= 4
                                ? 'CoreAndroid'
                                : 'App';
...
define("cordova/plugin/android/app", function(require, exports, module) {
  ...
  module.exports = {
     ...
     */
    clearCache :
        function() { exec(null, null, APP_PLUGIN_NAME, 'clearCache', []); },
     ...
     * dialog loadUrlTimeoutValue: int            => time in msec to wait before
     * triggering a timeout error clearHistory: boolean              => clear
     * webview history (default=false) openExternal: boolean              =>
     * open in a new browser (default=false)
     *
     * Example:
     ...
     */
    cancelLoadUrl :
        function() { exec(null, null, APP_PLUGIN_NAME, 'cancelLoadUrl', []); },
     ...
     */
    clearHistory :
        function() { exec(null, null, APP_PLUGIN_NAME, 'clearHistory', []); },
     ...
     */
    backHistory :
        function() { exec(null, null, APP_PLUGIN_NAME, 'backHistory', []); },
     ...
     * user registers for the "backbutton" event, this is automatically done.
     *
     ...
     *
     * Note: The user should not have to call this method.  Instead, when the
     * user registers for the "volume[up|down]button" event, this is
     * automatically done.
     *
     ...
     */
    exitApp :
        function() { return exec(null, null, APP_PLUGIN_NAME, 'exitApp', []); }
  };
});
...
define("cordova/pluginloader", function(require, exports, module) {
  ...
  // This function is only called if the really is a plugins array that isn't
  // empty. Otherwise the onerror response handler will just call
  // finishPluginLoading().
  ...
  // onPluginsReady is fired when there are no plugins to load, or they are all
  // done.
  exports.load = function(callback) {
    ...
    injectIfNecessary('cordova/plugin_list',
                      pathPrefix + 'cordova_plugins.js', function() {
...
define("cordova/utils", function(require, exports, module) {
  ...
  utils.typeName = function(
      val) { return Object.prototype.toString.call(val).slice(8, -1); };

  /**
   * Returns an indication of whether the argument is an array or not
   */
  utils.isArray =
      Array.isArray || function(a) { return utils.typeName(a) === 'Array'; };
  ...
  utils.clone = function(obj) {
    ...
    for (i in obj) {
      ...
      if ((!(i in retVal) || retVal[i] !== obj[i]) &&
          typeof obj[i] !== 'undefined' && typeof obj[i] !== 'unknown') {
